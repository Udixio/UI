...
export const PaletteToneRow: React.FC<Props> = ({ api, group }) => {
  const toneSteps = useMemo(() => Array.from({ length: 11 }, (_, i) => i * 10), []);
        ...
        {toneSteps.map((t) => {
          ...
          return (
            <div key={t} className="flex flex-col items-center">
              <div
                className="w-10 h-6 rounded border border-outline-variant/40"
                style={{ background: hex, color: textColor }}
                title={`tone ${t} ${hex}`}
              />
              <div className="mt-1 text-xs opacity-70">{t}</div>
            </div>
          );
        })}
      </div>
