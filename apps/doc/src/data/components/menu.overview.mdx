...
/>
        ...
        <AnchorPositioner anchorRef={chipRef} position="bottom left" className="mt-1">
             <Menu
                selected={selected}
                onItemSelect={(val) => {
                    setSelected(val);
...
function ContextMenu() {
  ...
  const handleContextMenu = (event) => {
    console.log('context menu triggered', {mouseX: event.clientX,
  ...
  if (!contextMenu) return;
    ...
    window.addEventListener('contextmenu', handleOutsideInteraction);

    return () => {
        window.removeEventListener('click', handleOutsideInteraction);
        window.removeEventListener('scroll', handleOutsideInteraction);
        window.removeEventListener('contextmenu', handleOutsideInteraction);
    };

  }, [contextMenu]);

  return (
  <div onContextMenu={handleContextMenu} className="cursor-context-menu relative  flex items-center justify-center">
