---
---















import { CodePreview } from '@/components/CodePreview.js';
import { iAdd } from '@udixio/icons-rounded-400/add';

## Usage

Floating Action Button (Fab) represents the primary action on a screen. Import it from @udixio/ui-react.

```bash
import { Fab } from "@udixio/ui-react"
```

<CodePreview client:load
             code={`
  <div className="flex items-end gap-4 h-96">
    <FabMenu size="large" label="Create" icon={iAdd} >
      <Button variant="filled">Filled</Button>
      <Button variant="elevated">Elevated</Button>
      <Button variant="tonal">Tonal</Button>
      <Button variant="outlined">Outlined</Button>
      <Button variant="text">Text</Button>
    </FabMenu>
  </div>`}
             scope={{ iAdd }}
/>

<CodePreview client:load
             code={`
  <div className="flex items-end gap-4 h-96">
    <FabMenu extended variant="secondary" size="large" label="Create" icon={iAdd} >
      <Button variant="filled">Filled</Button>
      <Button variant="elevated">Elevated</Button>
      <Button variant="tonal">Tonal</Button>
      <Button variant="outlined">Outlined</Button>
      <Button variant="text">Text</Button>
    </FabMenu>
  </div>`}
             scope={{ iAdd }}
/>


<CodePreview client:load
             code={`
  <div className="flex items-end gap-4 h-96">
    <FabMenu variant="tertiary" size="large" label="Create" icon={iAdd} >
      <Button variant="filled">Filled</Button>
      <Button variant="elevated">Elevated</Button>
      <Button variant="tonal">Tonal</Button>
      <Button variant="outlined">Outlined</Button>
      <Button variant="text">Text</Button>
    </FabMenu>
  </div>`}
             scope={{ iAdd }}
/>
