...
Exemple non contrôlé (valeur initiale):
...
Suppression: si `variant="input"`, chaque chip est rendu amovible et `Chips` retire l’élément supprimé de la liste (et appelle `onItemsChange` en mode contrôlé).