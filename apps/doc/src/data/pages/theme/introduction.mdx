...
`@udixio/ui-react` re-exports the theming API you use in app code — `defineConfig`, `ThemeProvider` — so you rarely import from `@udixio/theme` directly. You still install it: it carries the generator that turns your config into CSS, which you wire into your build (`vitePlugin` and friends). See [Get started — React](/get-started/react) for the full build setup this page assumes.
