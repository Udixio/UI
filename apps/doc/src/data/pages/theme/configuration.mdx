...
The single color that drives the entire palette. Accepted as a hex string, a `Color`, or a dynamic function:

```ts
sourceColor: '#6750A4'

import { Color } from '@udixio/theme';
sourceColor: Color.from({ hue: 270, chroma: 36, tone: 49 })
...
```ts
palettes: {
  // Shift the primary hue. `rotate` normalises over 360°.
  primary: ({ sourceColor }) => sourceColor.rotate(30),
  // Soften the secondary
...
A palette is a hue and a chroma, so the tone of a `Color` returned here is
ignored.
```
...
Only the **hue** of each value is used — the chroma and tone always come from
`sourceColor`, so every sub-theme stays in step with the rest. A `Color` or a
bare hue number works as well as a hex string:
