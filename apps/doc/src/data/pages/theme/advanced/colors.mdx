...
Palette definitions are initialized by the theme engine with the current `API`
before they are read:

```ts
type ColorConfigRecord<Value> =
  Record<string, Value> | ((api: API) => Record<string, Value>);

type ColorsConfig = ColorConfigRecord<
  string | Color | ((current: Color) => Color)
>;
```

The runtime method `api.colors.addColors()` also accepts this flat collection.
When a value is a function, it receives the current token and is reapplied
lazily whenever the source color changes.
...
In `defineConfig`, the same value belongs directly under `colors`:
...
```ts
const highlight = Color.fromPalette('tertiary', {
  tone: () => 70,
...
| `chromaMultiplier` | Multiplies the palette chroma after the default color customizations. Default `1`. Read the applied result back as `color.options.chroma`.                                                                            |
...
`background` is a `ColorRef`: a token key, a `Color`, or a function without
arguments when the reference depends on the current context. The function is
evaluated lazily, so it can close over the `context` and `colors` available
when the configuration is declared:
