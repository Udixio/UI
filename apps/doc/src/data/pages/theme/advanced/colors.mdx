...
The `colors` option in `ConfigInterface` separates direct values from palette-based
definitions:

```ts
type ColorsConfig = {
  static?: ColorConfigRecord<string | Color | { alias: string }>;
...
The runtime method `api.colors.addColors()` keeps its flat `AddColorsOptions`
form. The configuration form makes the static or dynamic nature of each token
explicit.
...
In `defineConfig`, the same value belongs under `colors.static`:
...
| `chromaMultiplier` | Multiplies the palette chroma. Default `1`. Read the applied result back as `color.options.chroma`. |
