...
The callback is re-evaluated on every `api.load()`. It receives the current `Context`, whose `sourceColor` is a `Color`. A `Color` has `hue` and `chroma`, so returning one directly works — its tone is ignored, since a palette spans every tone.

```ts
import { Color } from '@udixio/theme';

// Returning a Color — `rotate` normalises the hue over 360°
const callback: PaletteCallback = ({ sourceColor }) => sourceColor.rotate(30);

// Or a plain literal, when the maths does not map onto a derivation
