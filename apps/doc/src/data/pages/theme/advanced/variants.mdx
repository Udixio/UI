...
```ts
import { Color, variant, defaultColors } from '@udixio/theme';
...
| `colors`                  | `ColorsConfig`                        | —        | The tokens themselves. Pass `defaultColors` for the full Material set.         |
...
The tokens themselves. Passing `defaultColors` gives you the full Material set, resolved against your palettes — which is what the four standard variants do, differing only in their palettes. Each value follows the same `ColorsConfig` contract as the theme configuration: a hex string, a `Color`, or a transformation of an existing colour.

To change a few tokens, spread the defaults and override:

```ts
import { Color, defaultColors, contrastAgainst, avoidBackgroundGap } from '@udixio/theme';
