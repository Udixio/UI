...
The engine expects `primary`, `secondary`, `tertiary`, `neutral`, `neutralVariant` and `error` if you reuse `Variants.TonalSpot.colors`, because those are the palettes it reads. Define fewer only if you also supply your own `colors`.
