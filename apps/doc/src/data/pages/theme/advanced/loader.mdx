...
import { FontPlugin, Variants } from '@udixio/theme';
...
| --------------- | --------------------------------------------- | ----------------------------- |
| `sourceColor`   | `string \| Color \| (ctx) => string \| Color` | Drives all palette generation |
...
`@udixio/theme` selects its Node entry automatically in Node-based tooling:

```ts
import { loader } from '@udixio/theme';
```

Use this in scripts that run outside a browser environment; the package's
Node export is selected by the runtime.
