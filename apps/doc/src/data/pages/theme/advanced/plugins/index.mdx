...
Plugins transform computed tokens into concrete outputs (CSS, JSON, etc.). They are initialized once, and their `onLoad()` hook runs when `api.load()` is called. `defineConfig` wires two plugins for the standard Tailwind setup: [TailwindPlugin](/theme/advanced/plugins/tailwind) and [FontPlugin](/theme/advanced/plugins/font).
...
class MyPluginImpl extends PluginImplAbstract<MyPluginOptions> {
  ...
  onLoad() {
    ...
    const primary = this.api.colors.get('primary').hex;
...
| `this.api.palettes` | `PaletteApi` | Computed palettes (`get`, `add`, `override`, `sync`)       |
| `this.api.colors`   | `ColorApi`   | Semantic tokens (`get`, `getAll`, `addColor`, `addColors`) |
