...
> **Déprécié.** `@udixio/mcp` embarque un instantané figé de la doc et du thème au moment de son
> build, qui se périme à chaque évolution de la librairie. Pour connecter un agent IA à Udixio UI
> aujourd'hui, utilisez plutôt le [plugin Claude Code/Codex ou la doc en markdown](/agents), qui
> lisent toujours des données à jour. Ce guide reste disponible pour les clients MCP qui n'ont pas
> d'autre option d'intégration.
