...
`sourceColor` is the only required option. Unlike React — where `defineConfig` from `@udixio/ui-react` wires them for you — the two plugins are declared explicitly here. `outFile` is where the generated stylesheet is written; the generator also adds it to a sibling `.gitignore`, since it's a build artifact.
...
|------|-------------------|
| [Theme — Configuration](/theme/configuration) | Every config option (React-flavored, but the options are shared) |
