...
- Some components are delivered as attachments (`{@attach tooltip(() => ({ text: '…' }))}`) rather than components when their job is to attach behavior to an element you own — the docs page states the shape. Attachments are imported from the same package barrel and read their options through a function, so pass state inside it.
