...
1. Detect the framework from the project's `package.json`: `@udixio/ui-react`, `@udixio/ui-angular` or `@udixio/ui-svelte`.
2. Collect every Udixio component used in the files under review (imports from any of the packages, and — Angular only — the selectors those classes render under).
3. For each distinct component, read its installed API exactly as `consume-component` does: the shipped `.d.ts` (`node_modules/@udixio/ui-react/dist/…`, `node_modules/@udixio/ui-angular/index.d.ts` for Angular's single root type-entry, or `node_modules/@udixio/ui-svelte/dist/…` for Svelte), including the `@status`, `@parent`, `@devx`, `@a11y` and `@limitations` TSDoc tags.
...
- **Controlled/uncontrolled confusion** — passing a controlled value (`checked`, `value`, `pressed`) together with its `default*` counterpart on the same instance; or passing a controlled value with no change handler wired (and, in Svelte, no `bind:`), which silently freezes the component.
- **Svelte event idiom mistakes** — `on:valuechange`/`event.detail` on a component callback (they are plain callback props: `onValueChange={…}`), or legacy `<slot>`/`slot="…"` content where a snippet is expected.
