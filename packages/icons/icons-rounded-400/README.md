...
[Material Symbols](https://fonts.google.com/icons) icons as tree-shakable SVG strings — **rounded** style, weight **400**.
...
```bash
pnpm add @udixio/icons-rounded-400
```
...
```ts
import { iHomeFilled } from '@udixio/icons-rounded-400/filled/home'
...
Browse all available icons at [Material Symbols](https://fonts.google.com/icons?icon.set=Material+Symbols&icon.style=Rounded&icon.weight=400).
